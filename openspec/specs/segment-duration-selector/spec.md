...
`ShotDetail` 的时长展示元素 SHALL 支持用户点击后弹出选择器，在 `durationOptions`（由视频后端能力解析得到的可选时长列表）之间切换，选中后通过 `onUpdatePrompt` 回调将新值写入后端，并在保存完成后刷新剧集总时长。当 `durationOptions` 为连续整数区间且选项数较多时，选择器以滑块形式呈现。

#### Scenario: 点击时长徽章弹出选择器

- **WHEN** 用户点击时长徽章（如"4s"）
- **THEN** 弹出 Popover，按 `durationOptions` 列出可选时长按钮（或滑块），当前值高亮显示
...
- **WHEN** 未提供 `onUpdatePrompt` prop（只读模式）
